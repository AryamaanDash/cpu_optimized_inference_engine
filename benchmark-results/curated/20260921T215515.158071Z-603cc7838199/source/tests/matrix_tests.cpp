...
using Matmul = Matrix (*)(const Matrix&, const Matrix&);
...
void matmul_square(Matmul multiply) {
  Matrix a(2, 2);
  Matrix b(2, 2);
...
void matmul_rectangular(Matmul multiply) {
...
void matmul_incompatible_shapes(Matmul multiply) {
  const Matrix a(2, 3);
  const Matrix b(2, 3);
...
void matmul_zero_inner_dimension(Matmul multiply) {
  const Matrix a(2, 0);
  const Matrix b(0, 3);
...
void check_product(Matmul multiply, const Matrix& a, const Matrix& b, const Matrix& expected) {
...
void matmul_rectangular_output(Matmul multiply) {
    ...
    check_product(multiply, a, b, expected);
}

void matmul_scalar(Matmul multiply) {
    check_product(multiply, matrix_from_values(1, 1, {-2.5f}),
                  matrix_from_values(1, 1, {4.0f}),
                  matrix_from_values(1, 1, {-10.0f}));
}

void matmul_dot_product(Matmul multiply) {
    // 1 * 4 + (-2) * 5 + 3 * (-6) = -24.
    check_product(multiply, matrix_from_values(1, 3, {1, -2, 3}),
                  matrix_from_values(3, 1, {4, 5, -6}),
                  matrix_from_values(1, 1, {-24}));
}

void matmul_outer_product(Matmul multiply) {
    check_product(multiply, matrix_from_values(3, 1, {2, -1, 0.5f}),
...
void matmul_mixed_signs_and_fractions(Matmul multiply) {
    check_product(multiply, matrix_from_values(2, 3, {1, -2, 0.5f, 0, 3, -1}),
                  matrix_from_values(3, 5, {
                       2,  0, -1, 4,  1,
                       1, -2,  3, 0,  2,
                      -2,  4,  0, 1, -6
    ...
    check_product(multiply, matrix_from_values(1, 3, {0.1f, 0.2f, -0.3f}),
                  matrix_from_values(3, 2, {1, 2, 1, -1, 1, 0}),
                  matrix_from_values(1, 2, {0, 0}));
}

void matmul_identity(Matmul multiply) {
    ...
    check_product(multiply, identity_2, a, a);
    check_product(multiply, a, identity_3, a);
}

void matmul_zero_operands(Matmul multiply) {
    const Matrix a = matrix_from_values(2, 3, {1, -2, 0.5f, 0, 3, -1});
    check_product(multiply, a, Matrix(3, 5), Matrix(2, 5));
    check_product(multiply, Matrix(4, 2), a, Matrix(4, 3));
}

void matmul_empty_outputs(Matmul multiply) {
    check_product(multiply, Matrix(0, 3), Matrix(3, 5), Matrix(0, 5));
    check_product(multiply, Matrix(2, 3), Matrix(3, 0), Matrix(2, 0));
    check_product(multiply, Matrix(0, 3), Matrix(3, 0), Matrix(0, 0));
    check_product(multiply, Matrix(0, 0), Matrix(0, 0), Matrix(0, 0));
}

void matmul_incompatible_empty_shapes(Matmul multiply) {
...
// Conservative forward-error bound for bounded, normal FP32 inputs and ordinary
// round-to-nearest arithmetic (no fast-math). At most 2*K roundings per dot
// product covers separate multiply/add as well as FMA. Double products of these
// FP32 fixtures are exact; account for double summation and for rounding in the
// computed sum of absolute products. Compare directly with double, without a
// final FP32 rounding of the oracle. This remains meaningful under cancellation.
double dot_error_bound(std::size_t k, double sum_abs) {
    const double steps = 2.0 * static_cast<double>(k);
    const auto gamma = [steps](double unit_roundoff) {
        const double product = steps * unit_roundoff;
        check(product < 1.0, "Reduction exceeds error-bound domain");
        return product / (1.0 - product);
    };
    const double float_gamma = gamma(std::numeric_limits<float>::epsilon() / 2.0);
    const double double_gamma = gamma(std::numeric_limits<double>::epsilon() / 2.0);
    return (float_gamma + double_gamma) * sum_abs / (1.0 - double_gamma);
}

void check_dot(double actual, double expected, double sum_abs, std::size_t k) {
    const double allowed = dot_error_bound(k, sum_abs);
    if (!nearly_equal(actual, expected, allowed, 0.0)) {
        std::ostringstream message;
        message.precision(17);
        message << "Dot product mismatch: actual=" << actual << ", expected=" << expected
                << ", absolute_error=" << std::abs(actual - expected)
                << ", allowed_error=" << allowed << ", sum_abs=" << sum_abs << ", K=" << k;
        throw std::runtime_error(message.str());
    }
}

void dot_error_bound_checks() {
    check_dot(0.0, 0.0, 0.0, 0);
    check(dot_error_bound(1024, 100.0) > dot_error_bound(256, 100.0),
          "Error bound must reflect reduction length");
    check(dot_error_bound(256, 200.0) > dot_error_bound(256, 100.0),
          "Error bound must reflect absolute product sum");
    const double bound = dot_error_bound(1024, 1024.0);
    check_dot(bound / 2.0, 0.0, 1024.0, 1024);
    expect_throw<std::runtime_error>(
        [&] { check_dot(bound * 2.0, 0.0, 1024.0, 1024); },
        "Error bound accepted excessive cancellation error");
    expect_throw<std::runtime_error>(
        [] { check_dot(0.001, 0.0, 0.0, 1024); },
        "Zero products accepted nonzero output");
    expect_throw<std::runtime_error>(
        [] { check_dot(std::numeric_limits<double>::quiet_NaN(), 0.0, 1.0, 256); },
        "Error bound accepted NaN");
}

void matmul_large_reductions(Matmul multiply) {
    struct Shape { std::size_t m, k, n; };
    // All seven benchmark shapes, plus awkward and longer reductions in both
    // vector orientations and a non-square matrix case. Avoid a large cubic grid.
...
int main() {
    ...
    const KernelTest kernel_tests[] = {
        {"matmul 48 benchmark and long-reduction oracle cases", matmul_large_reductions},
    ...
    for (const auto& implementation : {
        ...
        for (const auto& test : kernel_tests) {
            try {
                ...
                std::cout << "PASS: " << implementation.name << ": " << test.name << '\n';
            } catch (const std::exception& error) {
                ++failures;
                std::cerr << "FAIL: " << implementation.name << ": " << test.name
                          << ": " << error.what() << '\n';
            } catch (...) {
                ++failures;
                std::cerr << "FAIL: " << implementation.name << ": " << test.name
